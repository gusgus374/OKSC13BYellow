...
with st.expander("Catapult Login Info",icon=":material/passkey:"):
    st.subheader("Email Address: yellow13b@datarook.com")
    st.subheader("password: YellowPool2024")
...
if season_choice == "Fall 24":
        ...
        game_data["day"] = game_data["Date"] - 45528
        game_data = game_data.loc[game_data["day"] > 0]
if season_choice == "Fall 23 and Spring 24":
        file_path = './data/OneKnox_13B_Yellow_full2324Season.csv'
        all_data = pd.read_csv(file_path)
        all_data.columns = [col.replace(':', ' ') for col in all_data.columns]
        #keep only the rows were the column 'Split Name' has a value equal to 'all'
        game_data = all_data.loc[all_data['Split Name'] == "game"]
...
with st.expander(label="View Your Data",expanded=False):
    ...
    st.write(game_data)
variable_x = st.selectbox("Pick Your X Variable!",game_data.columns.to_list(),10)
...
if variable_x == 'Date':
    ...
    tooltip=["Player Name","Session Title","Power Plays","Top Speed (m/s)","Distance Per Min (m/min)","Work Ratio","Max Deceleration (m/s/s)","Max Acceleration (m/s/s)"]).properties(height=500).interactive()
else:
    chart = alt.Chart(game_data).mark_circle().encode(
    x=variable_x,
    y=variable_y,
